...
- 点击日志跳转到编辑器里的真实位置
- 查看某条日志的源码、行号、来源标识（`Plugin generated` 或 `User written`）、是否被保护
- 配置当前文件快捷清除范围：只清除插件生成日志，或清除所有日志
...
默认情况下，这个快捷键只清除由 Log Manager 快速生成的日志，不会清除用户自己手写的 `console.*`。如果你想让快捷键清除当前文件里的所有 `console.*`，可以在侧边栏 Dashboard 的 `Quick clear scope` 中切换为 `All logs`。
...
- `logManager.currentFileCleanupScope`：当前文件快捷清除范围，`generated` 只清除插件生成日志，`all` 清除所有日志
...
- Inspect source, line number, source labels (`Plugin generated` or `User written`), and preserved state
- Configure the current-file shortcut cleanup scope: generated logs only, or all logs
...
By default, this shortcut removes only logs generated by Log Manager, so manually written `console.*` statements are left alone. To remove every `console.*` in the current file with the shortcut, switch `Quick clear scope` to `All logs` in the side bar dashboard.
...
- `logManager.currentFileCleanupScope`: current-file shortcut cleanup scope; `generated` removes only plugin-generated logs, `all` removes every log
